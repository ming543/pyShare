...
def mMenu():
    ...
    m1 = '複製全部OS印象檔至測試碟 Sync all OS image to test disk'
    m2 = '複製單一OS印象檔至測試碟 Choose one OS image to test disk'
    m3 = '更新本機OS_TESTER資料夾 Update OS_TESTER local folder'
    m4 = '更新本機OS_IMAGE資料夾 Update OS_IMAGE local folder'
    m6 = '本機系統更新 System Update'
    m7 = '更新本機測試程式 Update Test Script'
    ml = '系統關機 Power off system'
    options = [m0, m1, m2, m3, m4, m6, m7, ml]
    ...
    if choice == m0:  # Build test disk
        testerBuild()
    if choice == m1:  # Sync all OS
        osSyncAll()
    if choice == m2:  # Sync single OS
        osSyncOne()
    if choice == m3:  # Sync single OS
        ostesterSyncLocal()
    if choice == m4:  # Sync OS_IMAGE
        osimageSyncLocal()
    if choice == m6:  # System update
        systemUpdate()
...
#    print("按任意鍵關機,n鍵返回主選單  ", end='')
    print("按任意鍵返回主選單  ", end='')
    check = input("Press any key back to menu").lower()
    mMenu()
#    if check == ("n"):
#        mMenu()
#    os.system('systemctl poweroff')
...
def osSyncOne():
    ...
    if labelName == "U3_PROG":
        subprocess.check_call("sudo mount /dev/%s5 /mnt" % diskGet, shell=True, stdin=sys.stdin)
        subprocess.check_call("sudo rsync -avh /home/partimag/OS_IMAGE/%s /mnt/home/partimag/OS_IMAGE/" % osGet, shell=True, stdin=sys.stdin)
        subprocess.check_call("sudo umount /mnt", shell=True, stdin=sys.stdin)
        subprocess.check_call("sync", shell=True, stdin=sys.stdin)
    elif labelName == "DOS_G20B":
        subprocess.check_call("sudo mount /dev/%s2 /mnt" % diskGet, shell=True, stdin=sys.stdin)
        subprocess.check_call("sudo rsync -avh /home/partimag/OS_IMAGE/%s /mnt/OS_IMAGE/" % osGet, shell=True, stdin=sys.stdin)
        subprocess.check_call("sudo umount /mnt", shell=True, stdin=sys.stdin)
        subprocess.check_call("sync", shell=True, stdin=sys.stdin)
    elif labelName == "BOOT":
        subprocess.check_call("sudo mount /dev/%s2 /mnt" % diskGet, shell=True, stdin=sys.stdin)
...
def ostesterSyncLocal():
    syncFolder = "/home/partimag/OS_TESTER"
    if os.path.isdir(syncFolder):
        print("syncfolder exist")
    else:
        print(Fore.RED + "更新失敗 Can not find OS_TESTER folder!!!" + Fore.RESET)
        input("按任意鍵繼續 Press any key continue...")
        #break
        #os.makedirs(os.path.dirname(syncFolder), exist_ok=True)  # Create sync folder
    os.system('clear')
    for i in range(5):  # ping 5 times
        response = subprocess.call(
                "ping -c 1 -w 1 8.8.8.8", shell=True)
        if response == 0:
            print("PING OK")
            pS = subprocess.call(
                    "rclone -v sync pelog:OS_TESTER/ %s -L -P" % syncFolder, shell=True)
            if pS == 0:
                print("osPullDone")
...
def osimageSyncLocal():
    syncFolder = "/home/partimag/OS_IMAGE"
    if os.path.isdir(syncFolder):
        print("syncfolder exist")
    else:
        print(Fore.RED + "更新失敗 Can not find OS_IMAGE folder!!!" + Fore.RESET)
        input("按任意鍵繼續 Press any key continue...")
        #break
        #os.makedirs(os.path.dirname(syncFolder), exist_ok=True)  # Create sync folder
    ...
    for i in range(5):  # ping 5 times
        ...
        if response == 0:
            print("PING OK")
            pS = subprocess.call(
                    "rclone -v sync pelog:OS_IMAGE/ %s -L -P" % syncFolder, shell=True)
            if pS == 0:
                print("osPullDone")
                print(Fore.GREEN + "更新成功 Update done!!!" + Fore.RESET)
                input("按任意鍵繼續 Press any key continue...")
...
def cmEfcoSyncLocal():
    ...
    for i in range(5):  # ping 5 times
        ...
        if response == 0:
            print("PING OK")
            pS = subprocess.call(
                    "rclone -v sync CM_EFCO:U7-XXX %s/U7-XXX/ --exclude=/.git/** --exclude=/BOM/** -L -P" % syncFolder, shell=True)
            if pS == 0:
